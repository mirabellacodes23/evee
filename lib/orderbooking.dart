            ...
            child: pw.ConstrainedBox(
              ...
              child: pw.Table(
                ...
                children: [
                  ...
                  if (isInstallment) ...<pw.TableRow>[
                    row(
                      'Total Amount Rs.',
                      totalAmount ?? digitsOnly(orderData['bikePrice']),
                    ),
                    row(
                      'Down Payment Rs.',
                      (orderData['downPayment'] ?? 0).toString(),
                    ),
                    row('Current Installment Status', () {
                      final paymentHistory =
                          orderData['paymentHistory'] as List<dynamic>?;
                      final completedInstallments =
                          paymentHistory
                              ?.where((p) => p['type'] == 'installment')
                              .length ??
                          0;
                      final totalInstallments =
                          orderData['totalInstallments'] ?? 0;
                      return 'Installment #$completedInstallments/$totalInstallments';
                    }()),
                    row('Total Paid Rs.', paidAmount ?? '0'),
                    row(
                      'Remaining Amount Rs.',
                      remainingAmount ??
                          totalAmount ??
                          digitsOnly(orderData['bikePrice']),
                    ),
                    row('Payment Mode', 'Installment'),
                  ] else ...<pw.TableRow>[
                    row('Amount Rs.', digitsOnly(orderData['bikePrice'])),
...
class _OrderPageState extends State<OrderPage> {
  ...
  Future<int> _getCurrentSeq(String field) async {
    final ref = FirebaseFirestore.instance.collection('meta').doc('counters');
    final snap = await ref.get();
    final data = snap.data() ?? <String, dynamic>{};
    return (data[field] is num) ? (data[field] as num).toInt() : 1;
  }
...
''';
  ...
  Future<void> _openInquiryDialog(
    ...
    Map<String, String>? findBy(String key, String? value) {
    ...
    // Custom date for invoice/challan
    DateTime customDate = DateTime.now();

    // Invoice and challan number controllers (use current, don't increment)
    final invoiceSeq = await _getCurrentSeq('invoice');
    final chalanSeq = await _getCurrentSeq('chalan');
    final invoiceNumCtrl = TextEditingController(text: _fmtInvoice(invoiceSeq));
    final chalanNumCtrl = TextEditingController(text: _fmtChalan(chalanSeq));

    // Total installments controller (if installment order)
    final isInstallment = orderData['isInstallment'] == true;
    final totalInstallmentsCtrl = TextEditingController(
      text: (orderData['totalInstallments'] ?? 6).toString(),
    );
            ...
            return _GlassDialog(
              ...
              content: Column(
                ...
                children: [
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        onChanged: (value) {
                          ...
                          syncFromVariant(findBy('chassis', value), setState);
                        },
                      ),
                    ),
                  ),
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        onChanged: (value) {
                          ...
                          syncFromVariant(findBy('engine', value), setState);
                        },
                      ),
                    ),
                  ),
                  ...
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        onChanged: (value) {
                          selectedColor = value;
                          syncFromVariant(findBy('color', value), setState);
                        },
                      ),
                    ),
                  ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text(
                                '${customDate.day.toString().padLeft(2, '0')}-${customDate.month.toString().padLeft(2, '0')}-${customDate.year}',
                                style: const TextStyle(
                                  fontWeight: FontWeight.w700,
                                  fontSize: 16,
                                  color: Colors.blue,
                  ...
                  // Total Installments Field (only if installment order)
                  if (isInstallment) ...[
                    const SizedBox(height: 12),
                    Container(
                      padding: const EdgeInsets.symmetric(
                        ...
                        child: OutlinedButton.icon(
                          onPressed:
                              ...
                              ? () async {
                                  ...
                                  // Update total installments if installment order
                                  if (isInstallment) {
                                    final newTotal =
                                        int.tryParse(
                                          totalInstallmentsCtrl.text.trim(),
                                        ) ??
                                        6;
                                    updateData['totalInstallments'] = newTotal;
                                  }
                                  ...
                                  // Update orderData with new values for PDF generation
                                  if (isInstallment) {
                                    orderData['totalInstallments'] =
                                        int.tryParse(
                                          totalInstallmentsCtrl.text.trim(),
                                        ) ??
                                        6;
  ...
  // ---------------- Add Payment Dialog (for installment orders) ----------------
  Future<void> _addPaymentDialog(
    ...
    final currentPaid = orderData['paidAmount'] ?? 0;
    final remaining = orderData['remainingAmount'] ?? 0;
    final total = orderData['totalAmount'] ?? 0;
    final downPayment = orderData['downPayment'] ?? 0;
    final currentInstallments = orderData['totalInstallments'] ?? 0;
    final nextInstallmentNumber = currentInstallments + 1;
        ...
        return _GlassDialog(
          title: Row(
            children: const [
              ...
              SizedBox(width: 8),
              Flexible(child: Text('Add Payment')),
            ],
          ),
          content: Form(
            key: formKey,
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text('Installments Paid: $currentInstallments'),
                          Text(
                            'Rs. ${currentPaid - downPayment}',
                            style: const TextStyle(fontWeight: FontWeight.w600),
                          ),
                        ],
                      ),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text(
                        downPayment == 0
                            ? 'This will be Down Payment (DP)'
                            : 'This will be Installment #$nextInstallmentNumber',
                        style: const TextStyle(
                          fontWeight: FontWeight.w700,
                          color: Colors.blue,
                        ),
                ...
                _FxTextField(
                  ...
                  label: downPayment == 0
                      ...
                      : 'Installment #$nextInstallmentNumber Amount (Rs.)',
                ...
                const SizedBox(height: 12),
                const Text(
                  '💡 This will update the paid amount and recalculate the remaining balance.',
                  style: TextStyle(
                    fontSize: 12,
          ...
          actions: [
            ...
            _PrimaryButton(
              ...
              onTap: () async {
                ...
                final paymentAmount = int.parse(
                  paymentCtrl.text.replaceAll(RegExp(r'[^0-9]'), ''),
                );
                ...
                final paymentType = isDownPayment
                    ? 'down_payment'
                    : 'installment';
                final installmentNum = isDownPayment
                    ? 0
                    : nextInstallmentNumber;
                final paymentNote = isDownPayment
                    ? 'Down Payment'
                ...
                try {
                  final updateData = {
                    ...
                    'paymentStatus': newRemaining > 0 ? 'partial' : 'paid',
                    'lastPaymentDate': FieldValue.serverTimestamp(),
                    'lastPaymentAmount': paymentAmount,
                    'paymentHistory': FieldValue.arrayUnion([paymentRecord]),
                  };

                  // If this is down payment, also update downPayment field
                  if (isDownPayment) {
                    updateData['downPayment'] = paymentAmount;
                  } else {
                    updateData['totalInstallments'] = nextInstallmentNumber;
                  }

                  await _ordersCol.doc(orderId).update(updateData);
                  ...
                  if (context.mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          'Payment of Rs.$paymentAmount added successfully!',
                        ),
                      ),
                    );
                  }
  ...
  Future<void> _showPaymentHistoryDialog(
    ...
    final paymentHistory =
        (orderData['paymentHistory'] as List?)?.cast<Map<String, dynamic>>() ??
        [];
        ...
        return _GlassDialog(
          title: Row(
            children: const [
              ...
              SizedBox(width: 8),
              Flexible(child: Text('Payment History')),
            ],
          ),
          content: Column(
            mainAxisSize: MainAxisSize.min,
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        const Text('Remaining:'),
                        Text(
                          'Rs. ${orderData['remainingAmount'] ?? 0}',
                          style: const TextStyle(
                            fontWeight: FontWeight.w700,
                            color: Colors.orange,
                          ),
                ...
                ConstrainedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      final installmentNum = payment['installmentNumber'] ?? 0;
                      ...
                      final isDownPayment = type == 'down_payment';
                        ...
                        child: Row(
                          children: [
                            Container(
                              ...
                              child: Center(
                                child: Text(
                                  isDownPayment ? 'DP' : '#$installmentNum',
                                  style: const TextStyle(
                                    fontWeight: FontWeight.w800,
                                    fontSize: 12,
                                    color: Colors.white,
          ...
          actions: [
            _PrimaryButton(text: 'Close', onTap: () => Navigator.pop(context)),
          ],
        );
      },
    );
                                      ...
                                      children: [
                                        ...
                                        if (data['isInstallment'] ==
                                          ...
                                          Container(
                                            ...
                                            child: Text(
                                              'Installment: Paid Rs.${data['paidAmount']} / Total Rs.${data['totalAmount']} | Remaining: Rs.${data['remainingAmount']}',
                                              style: const TextStyle(
                                                fontSize: 11,
                                                fontWeight: FontWeight.w600,
                                                color: Colors.orange,
                                            ...
                                            child: Builder(
                                              builder: (context) {
                                                ...
                                                final completedInstallments =
                                                    ...
                                                    0;
                                                return Text(
                                                  'Down: Rs.${data['downPayment'] ?? 0} | Current: Installment #$completedInstallments/${data['totalInstallments'] ?? 0}',
                                                  style: const TextStyle(
                                                    fontSize: 11,
                                                    fontWeight: FontWeight.w600,
                                                    color: Colors.cyan,
...
class _InvoiceDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Installment details
    final isInstallment = orderData['isInstallment'] == true;
    final totalAmount = (orderData['totalAmount'] ?? 0).toString();
    final paidAmount = (orderData['paidAmount'] ?? 0).toString();
    final remainingAmount = (orderData['remainingAmount'] ?? 0).toString();
                      ...
                      child: ConstrainedBox(
                        ...
                        child: Table(
                          ...
                          children: [
                            ...
                            _row('Engine #', engine),
                            _row('Color', color),
                            if (isInstallment) ...<TableRow>[
                              _row('Total Amount Rs.', totalAmount),
                              _row(
                                'Down Payment Rs.',
                                (orderData['downPayment'] ?? 0).toString(),
                              ),
                              _row(
                                'Installments Paid',
                                '${orderData['totalInstallments'] ?? 0} payments',
                              ),
                              _row('Total Paid Rs.', paidAmount),
                              _row('Remaining Amount Rs.', remainingAmount),
                              _row('Payment Mode', 'Installment'),
                            ] else ...<TableRow>[
                        ...
                        child: Table(
                          ...
                          children: [
                            ...
                            if (orderData['isInstallment'] ==
                              ...
                              _row(
                                'Total Paid Rs.',
                                (orderData['paidAmount'] ?? 0).toString(),
                              ),
                              _row(
                                'Remaining Amount Rs.',
                                (orderData['remainingAmount'] ?? 0).toString(),
                              ),
                              _row('Payment Status', 'Installment'),
                            ],
                          ],
                        ),
                      ),
